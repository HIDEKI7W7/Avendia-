...
export function DocumentControls({ document, refresh }: { document: Document; refresh: () => Promise<void> }) {
  ...
  const query = useQuery({ queryKey: utilityKey("versions", document?.id), enabled: expanded && Boolean(document?.id), queryFn: ({ signal }) => utilityApi<Version[]>(`/documents/${document.id}/versions`, "GET", undefined, signal) });
  ...
  const isFavorite = Boolean(document?.favorite);
  return <div style={{ width: "100%" }}><button className="secondary-button" aria-expanded={expanded} onClick={() => setExpanded(!expanded)}>Organizar · versión {revision}</button>{expanded ? <div className="utilities-form"><form onSubmit={e => { e.preventDefault(); mutation.mutate({ body: { title, expected_revision: revision } }); }}><label>Nombre del documento<input value={title} onChange={e => setTitle(e.target.value)} required minLength={3} maxLength={240} /></label><button className="secondary-button" disabled={mutation.isPending}>Renombrar</button></form><button className="secondary-button" disabled={mutation.isPending} aria-pressed={isFavorite} onClick={() => mutation.mutate({ body: { favorite: !isFavorite, expected_revision: revision } })}>{isFavorite ? "Quitar favorito" : "Guardar favorito"}</button><h3>Versiones anteriores</h3>{query.isPending ? <p>Cargando versiones…</p> : query.isError ? <p role="alert">{errorText(query.error)}</p> : query.data?.length ? query.data.map(v => <details key={v.id}><summary>Versión {v.revision} · {v.title}</summary><pre style={{ whiteSpace: "pre-wrap", overflowWrap: "anywhere", maxHeight: 240, overflow: "auto" }}>{v.content ?? "Sin contenido"}</pre><button className="secondary-button" onClick={() => setRestore(v)}>Restaurar como nueva versión</button></details>) : <p>Las versiones aparecerán después de guardar cambios.</p>}{restore ? <div role="alert"><p>¿Restaurar la versión {restore.revision}? Se conservará la versión actual.</p><button className="secondary-button" disabled={mutation.isPending} onClick={() => mutation.mutate({ suffix: `/versions/${restore.id}/restore` })}>Confirmar restauración</button><button className="secondary-button" onClick={() => setRestore(null)}>Cancelar</button></div> : null}</div> : null}{mutation.isError ? <p role="alert">{errorText(mutation.error)}</p> : null}</div>;
}
...
export function DocumentTrash({ refresh }: { refresh: () => Promise<void> }) {
  ...
  return <section className="utility-panel utilities-page"><h2>Papelera de documentos</h2>{query.isPending ? <p>Cargando…</p> : query.isError ? <p role="alert">{errorText(query.error)}</p> : <>{total === 0 ? <p>No hay documentos en la papelera.</p> : items.map(d => <div className="utilities-actions" key={d.id}><span>{d.title}</span><button className="secondary-button" disabled={mutation.isPending} onClick={() => mutation.mutate(d.id)}>Recuperar</button></div>)}<div className="utilities-actions"><button disabled={page === 1} onClick={() => setPage(p => p-1)}>Anterior</button><button disabled={page*20 >= total} onClick={() => setPage(p => p+1)}>Siguiente</button></div></>}{mutation.isError ? <p role="alert">{errorText(mutation.error)}</p> : null}</section>;
}
